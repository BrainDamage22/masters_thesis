...
def test_delta1_for_ng_routing(path, amount, range_x, range_y, starting_node, iterations, lower_bound):
    for i in range(0, iterations):
        ...
        for delta1 in range(1, len(nodes) + 1):
            ...
            result = ng_routing(starting_node, node_objects.copy(), costs_list, lower_bound)
...
def test_delta1_and_delta2_for_dng_pathing(path, amount, range_x, range_y, starting_node, iterations, lower_bound):
    for i in range(0, iterations):
        create_data(amount, range_x, range_y, path + "temp/")
        costs_list, nodes = read_data(path + "temp/")
        ...
        for delta1 in range(1, len(nodes) + 1):
            for delta2 in range(1, len(nodes) + 1):
                if delta2 < delta1:
                    ...
                    node_objects = calculate_n_sets(costs_list, nodes, delta1)
                    result, dng_results = dynamic_ng_pathing(starting_node, node_objects.copy(), costs_list, delta2, lower_bound)
                    results.append(result)
